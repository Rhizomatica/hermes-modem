        ...
        for (size_t i = 0; i < read_len; i++)
        {
            ...
            if (byte == FEND)
            {
                if (in_kiss_frame && kiss_cmd == CMD_DATA && kiss_frame_pos > 0)
                {
                    ...
                    {
                        size_t chunk_size = (payload_to_send - offset > payload_per_frame) ? payload_per_frame : (payload_to_send - offset);
                        ...
                        memcpy(frame_buffer + HEADER_SIZE + len_field_size, kiss_frame_data + offset, chunk_size);
