...
typedef struct {
    ...
    bool running;
    bool started;
    int queue[ARQ_EVENT_QUEUE_CAPACITY];
    size_t head;
    size_t tail;
    size_t count;
    time_t last_tick_at;
...
static void arq_event_loop_enqueue(int event);
...
static void arq_event_loop_enqueue(int event)
{
    bool dispatch_direct = false;

    if (event < 0)
        return;

    pthread_mutex_lock(&arq_event_loop.lock);
    if (!arq_event_loop.started || !arq_event_loop.running || arq_event_loop.count >= ARQ_EVENT_QUEUE_CAPACITY)
    {
        dispatch_direct = true;
    }
    else
    {
        arq_event_loop.queue[arq_event_loop.tail] = event;
...
static void *arq_event_loop_worker(void *arg)
{
    ...
    for (;;)
    {
        ...
        {
            event = arq_event_loop.queue[arq_event_loop.head];
